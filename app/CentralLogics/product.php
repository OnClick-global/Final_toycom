...
class ProductLogic
{
    ...
    {
        return Product::active()->withCount(['wishlist'])->with(['rating','Ages'])->where('id', $id)->first();
    }

    public static function get_latest_products($limit = 10, $offset = 1)
    {
        $count = Product::active()->count();
        $paginator = Product::active()->withCount(['wishlist'])->with(['rating','Ages'])->latest()->paginate($count, ['*'], 'page', $offset);
        /*$paginator->count();*/
        return [
